...
import { env, isDev } from './runtimeEnv';
...
export const validateEnvironment = (): ValidationResult => {
  ...
  const has = (key: keyof EnvConfig) => Boolean(env(key as string));
  ...
  const firebaseMissing = firebaseVars.filter(v => !has(v as keyof EnvConfig));
  ...
  const supabaseMissing = supabaseVars.filter(v => !has(v as keyof EnvConfig));
  if (supabaseMissing.length > 0) {
    warnings.push(`Supabase not configured: ${supabaseMissing.join(', ')}`);
  }

  // Stripe validation
  if (!has('VITE_STRIPE_PUBLISHABLE_KEY')) {
  ...
  const crossSiteMissing = crossSiteVars.filter(v => !has(v as keyof EnvConfig));
  ...
  return {
    ...
    configured: {
      ...
      stripe: has('VITE_STRIPE_PUBLISHABLE_KEY'),
...
// Auto-validate on import in development (client only)
