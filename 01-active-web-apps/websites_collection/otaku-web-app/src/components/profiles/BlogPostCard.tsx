...
const BlogPostCard: React.FC<BlogPostCardProps> = ({ post, onEdit, onDelete }) => {
  ...
  return (
    <div className="glass-card rounded-xl overflow-hidden hover:border-violet-500 transition-all duration-300 shadow-lg hover:shadow-violet-900/20 group">
      {/* Cover Image */}
      {post.coverImage && (
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {post.tags.slice(0, 3).map(tag => (
            <span 
              key={tag}
              className="px-2 py-1 bg-[#050505] text-gray-400 text-xs rounded border border-[#26262a]"
            >
              #{tag}
            </span>
          ))}
